...
const removePartnersService = async ({ id }) => {
  // Banner mavjudligini tekshirish
  const existing = await Partners.findById(id);
  ...
  let delProd = await Partners.findByIdAndDelete(id);

  return delProd;
};

module.exports = removePartnersService;
